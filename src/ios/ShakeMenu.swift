...
extension CordovaUpdaterPlugin: UIGestureRecognizerDelegate {
...
extension UIWindow {
    override open func motionEnded(_ motion: UIEvent.EventSubtype, with event: UIEvent?) {
        if motion == .motionShake {
            guard let bridgeViewController = rootViewController as? CAPBridgeViewController,
                  ...
                  let plugin = bridge.plugin(withName: "CapacitorUpdater") as? CordovaUpdaterPlugin else {
                return
            }
            guard plugin.shakeMenuGesture == CordovaUpdaterPlugin.shakeMenuGestureShake else {
                return
            }
    ...
    @discardableResult
    fileprivate func showCapacitorUpdaterMenu(plugin: CordovaUpdaterPlugin, bridge: CAPBridgeProtocol, gestureName: String) -> Bool {
    ...
    @discardableResult
    private func showDefaultMenu(plugin: CordovaUpdaterPlugin, bridge: CAPBridgeProtocol) -> Bool {
    ...
    private func showConfiguredDefaultMenu(plugin: CordovaUpdaterPlugin, bridge: CAPBridgeProtocol) {
        let appName = Bundle.main.infoDictionary?["CFBundleDisplayName"] as? String ?? "App"
        let title = "Preview \(appName) Menu"
    ...
    private func showPreviewSelector(plugin: CordovaUpdaterPlugin) {
        guard let topVC = UIApplication.topViewController() else {
            return
        }
        if topVC.isKind(of: UIAlertController.self) {
    ...
    private func showSearchablePreviewPicker(previews: [[String: Any]], plugin: CordovaUpdaterPlugin) {
    ...
    private func presentPreviewPicker(previews: [[String: Any]], plugin: CordovaUpdaterPlugin) {
        let alert = UIAlertController(title: "Select Preview", message: "Choose a local preview to open", preferredStyle: .actionSheet)
    ...
    private func selectPreview(id: String, plugin: CordovaUpdaterPlugin) {
        DispatchQueue.global(qos: .userInitiated).async {
    ...
    @discardableResult
    private func showChannelSelector(plugin: CordovaUpdaterPlugin, bridge: CAPBridgeProtocol) -> Bool {
        // Prevent multiple alerts from showing
        guard let topVC = UIApplication.topViewController() else {
            return false
        }
    ...
    private func presentChannelPicker(channels: [[String: Any]], plugin: CordovaUpdaterPlugin, bridge: CAPBridgeProtocol) {
    ...
    private func showSearchableChannelPicker(channels: [[String: Any]], plugin: CordovaUpdaterPlugin, bridge: CAPBridgeProtocol) {
    ...
    private func selectChannel(name: String, plugin: CordovaUpdaterPlugin, bridge: CAPBridgeProtocol) {
        let updater = plugin.implementation
            ...
            if let topVC = UIApplication.topViewController() {
                topVC.present(progressAlert, animated: true) {
                    DispatchQueue.global(qos: .userInitiated).async {
                        ...
                        let updateUrlStr = pluginUpdateUrl.isEmpty ? CordovaUpdaterPlugin.updateUrlDefault : pluginUpdateUrl
    ...
    private func showError(message: String, plugin: CordovaUpdaterPlugin) {
    ...
    private func showSuccess(message: String, plugin: CordovaUpdaterPlugin) {

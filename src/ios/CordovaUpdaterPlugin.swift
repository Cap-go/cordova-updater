...
public class CordovaUpdaterPlugin: CDVPlugin, CDVPluginSchemeHandler {
    ...
    private func readConfigBool(_ key: String, _ defaultValue: Bool = false) -> Bool {
        guard let raw = readConfigString(key, nil) else { return defaultValue }
        return (raw as NSString).boolValue
    }

    private func readConfigInt(_ key: String, _ defaultValue: Int = 0) -> Int {
        guard let raw = readConfigString(key, nil) else { return defaultValue }
    ...
    lazy var logger: Logger = {
        ...
        let osLogging: Bool
        if true {
            osLogging = readConfigBool("osLogging", true)
        } else {
            osLogging = true
        }
    ...
    func loadPlugin() {
        ...
        persistCustomId = readConfigBool("persistCustomId", false)
        allowSetDefaultChannel = readConfigBool("allowSetDefaultChannel", true)
        persistDefaultChannelOnReinstall = readConfigBool("persistDefaultChannelOnReinstall", true)
        if persistCustomId {
        ...
        persistModifyUrl = readConfigBool("persistModifyUrl", false)
        allowManualBundleError = readConfigBool("allowManualBundleError", false)
        allowPreview = readConfigBool("allowPreview", false)
        logger.info("init for device \(self.implementation.deviceID)")
        guard let versionName = readConfigString("version", Bundle.main.versionName) else {
        ...
        implementation.versionBuild = readConfigString("version", Bundle.main.versionName)!
        autoDeleteFailed = readConfigBool("autoDeleteFailed", true)
        autoDeletePrevious = readConfigBool("autoDeletePrevious", true)
        keepUrlPathAfterReload = readConfigBool("keepUrlPathAfterReload", false)
        syncKeepUrlPathFlag(enabled: keepUrlPathAfterReload)

        autoSplashscreen = readConfigBool("autoSplashscreen", false)
        autoSplashscreenLoader = readConfigBool("autoSplashscreenLoader", false)
        let splashscreenTimeoutValue = readConfigInt("autoSplashscreenTimeout", 10000)
        autoSplashscreenTimeout = max(0, splashscreenTimeoutValue)
        updateUrl = readConfigString("updateUrl", CordovaUpdaterPlugin.updateUrlDefault)!
        ...
        appReadyTimeout = max(1000, readConfigInt("appReadyTimeout", 10000))  // Minimum 1 second
        implementation.timeout = Double(readConfigInt("responseTimeout", 20))
        resetWhenUpdate = readConfigBool("resetWhenUpdate", true)
        shakeMenuEnabled = readConfigBool("shakeMenu", false)
        shakeChannelSelectorEnabled = readConfigBool("allowShakeChannelSelector", false)
        shakeMenuGesture = Self.normalizedShakeMenuGesture(readConfigString("shakeMenuGesture", Self.shakeMenuGestureShake))
        ...
        implementation.statsUrl = readConfigString("statsUrl", CordovaUpdaterPlugin.statsUrlDefault)!
        implementation.channelUrl = readConfigString("channelUrl", CordovaUpdaterPlugin.channelUrlDefault)!
        if persistModifyUrl {
    ...
    private func initialLoad() -> Bool {
        ...
        schemeHandler?.activeBundleDir = URL(fileURLWithPath: dest.path)
        return true
    }
    ...
    private func getBooleanConfig(_ key: String, defaultValue: Bool) -> Bool {
        ...
        return readConfigBool(key, defaultValue)
    }
    ...
    private func getStringConfig(_ key: String, defaultValue: String) -> String {
        ...
        return readConfigString(key, defaultValue) ?? defaultValue
    }
    ...
    private func configureAutoUpdateModeFromConfig() {
        if let configuredMode = readConfigString("autoUpdate"),
           configuredMode != "",
           configuredMode != "true",
           configuredMode != "false" {
            ...
            let configuredMode = readConfigString("autoUpdate")
            let enabled = configuredMode != nil ? configuredMode == "true" : readConfigBool("autoUpdate", true)
    ...
    private func resolveLegacyDirectUpdateModeFromConfig() -> String {
        if let directUpdateString = readConfigString("directUpdate") {
        ...
        return readConfigBool("directUpdate", false) ? Self.autoUpdateModeAlways : "false"
    }

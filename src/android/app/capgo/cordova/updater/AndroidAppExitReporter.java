...
final class AndroidAppExitReporter {
    ...
    static void reportPreviousAppExitReasons(
        ...
        try {
            ...
            for (final ApplicationExitInfo exitInfo : exitReasons) {
                ...
                final String action = CordovaUpdaterPlugin.statsActionForApplicationExitReason(exitInfo.getReason());
                if (action == null) {
                    continue;
                }
    ...
    private static Map<String, String> buildApplicationExitMetadata(final ApplicationExitInfo exitInfo) {
        ...
        metadata.put("exit_reason", CordovaUpdaterPlugin.applicationExitReasonName(exitInfo.getReason()));
        ...
        if (processName != null && !processName.isEmpty()) {
            metadata.put("process_name", CordovaUpdaterPlugin.truncateStatsMetadataValue(processName, 128));
        }

        final String description = exitInfo.getDescription();
        if (description != null && !description.isEmpty()) {
            metadata.put("exit_description", CordovaUpdaterPlugin.truncateStatsMetadataValue(description, 512));
        }

...
public class ShakeMenu implements ShakeDetector.Listener, ThreeFingerPinchDetector.Listener {
    ...
    private CordovaUpdaterPlugin plugin;
    ...
    public ShakeMenu(CordovaUpdaterPlugin plugin, BridgeActivity activity, Logger logger, String gesture) {
        this.plugin = plugin;
        this.activity = activity;
        this.logger = logger;
        this.gesture = gesture;

        if (CordovaUpdaterPlugin.SHAKE_MENU_GESTURE_THREE_FINGER_PINCH.equals(gesture)) {
    ...
    private void showPreviewSelector() {
        activity.runOnUiThread(() -> {
            try {
                ...
                List<JSONObject> previews = new ArrayList<>();
                for (int i = 0; i < previewsRaw.length(); i++) {
                    Object raw = previewsRaw.opt(i);
                    if (raw instanceof JSONObject preview) {
                        previews.add(preview);
                    } else if (raw instanceof JSONObject json) {
                        previews.add(JSONObject.fromJSONObject(json));
                    }
                }
    ...
    private String previewLabel(JSONObject preview) {
        String name = preview.optString("name", "");
        JSONObject bundle = preview.getJSONObject("bundle");
    ...
    private void presentPreviewPicker(List<JSONObject> previews) {
        try {
            AlertDialog.Builder builder = new AlertDialog.Builder(activity);
            ...
            final List<JSONObject> displayedPreviews = new ArrayList<>();
            ...
            searchField.addTextChangedListener(
                new TextWatcher() {
                    ...
                    public void afterTextChanged(Editable s) {
                        ...
                        for (JSONObject preview : previews) {
            ...
            listView.setOnItemClickListener((parent, view, position, id) -> {
                JSONObject selectedPreview = displayedPreviews.get(position);
    ...
    private List<String> previewLabels(List<JSONObject> previews) {

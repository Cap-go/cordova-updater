...
final class NativeContractTests: XCTestCase {
    ...
    func testPeriodCheckDelayMatchesNativeContract() throws {
        for testCase in try contractCases("periodCheckDelay") {
            ...
            XCTAssertEqual(
                CordovaUpdaterPlugin.normalizedPeriodCheckDelaySeconds(try int(input, "seconds", id: id)),
    ...
    func testAutoUpdateModeMatchesNativeContract() throws {
        for testCase in try contractCases("autoUpdateMode") {
            ...
            let input = try dictionary(testCase, "input", id: id)
            let expect = try dictionary(testCase, "expect", id: id)
            let normalizedMode = CordovaUpdaterPlugin.normalizedAutoUpdateMode(try string(input, "mode", id: id))

            XCTAssertEqual(normalizedMode, try string(expect, "mode", id: id), id)
            XCTAssertEqual(CordovaUpdaterPlugin.isAutoUpdateModeEnabled(normalizedMode), try bool(expect, "enabled", id: id), id)
            XCTAssertEqual(
            ...
            XCTAssertEqual(
                CordovaUpdaterPlugin.shouldAutoUpdateModeSetNextBundle(normalizedMode),
    ...
    func testOnLaunchDirectUpdateConsumptionMatchesNativeContract() throws {
        for testCase in try contractCases("onLaunchDirectUpdateConsumption") {
            ...
            XCTAssertEqual(
                CordovaUpdaterPlugin.shouldConsumeOnLaunchDirectUpdate(
    ...
    func testUpdateResponseKindMatchesNativeContract() throws {
        for testCase in try contractCases("updateResponseKind") {
            ...
            XCTAssertEqual(
                CordovaUpdaterPlugin.normalizedUpdateResponseKind(kind: try optionalString(input, "kind", id: id)),
